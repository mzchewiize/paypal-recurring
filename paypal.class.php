<?php
	...
	class MyPayPal {
		...
		function SaveBuyerInfoIntoDatabase()
		{
		...
		function SetExpressCheckout($products, $charges, $noshipping='1'){
			...
			$padata .=  '&L_BILLINGAGREEMENTDESCRIPTION0=xxxx';
	...
	function SetCreateProfile(){
		...
		$padata .= 	'&L_BILLINGTYPE0=RecurringPayments';
		$padata .=  '&L_BILLINGAGREEMENTDESCRIPTION0=xxxx';
		$padata .=	'&PROFILESTARTDATE=2016-06-01T00:00:00Z';  
		$padata .=	'&DESC=xxxx';    
		$padata .=	'&BILLINGPERIOD=Day';   
		$padata .=	'&BILLINGFREQUENCY=1';    
		$padata .=	'&AMT=1';   
		$padata .=	'&CURRENCYCODE=USD';   
		$padata .=	'&COUNTRYCODE=US';   
		...
		function DoExpressCheckoutPayment(){
			...
			if(!empty(_SESSION('ppl_products'))&&!empty(_SESSION('ppl_charges'))){
				...
				$padata .= 	'&L_BILLINGTYPE0=RecurringPayments';
				$padata .=  '&L_BILLINGAGREEMENTDESCRIPTION0=xxxx';
				$pdadta .=  '&PROFILESTARTDATE=2016-06-01T00:00:00Z';    #Billing date start, in UTC/GMT format
				$pdadta .=  '&DESC=FitnessMembership';    #Profile description - same as billing agreement description
				$pdadta .=  '&BILLINGPERIOD=Day';    #Period of time between billings
				$pdadta .=  '&BILLINGFREQUENCY=1';    #Frequency of charges
				$pdadta .=  '&AMT=1';    #The amount the buyer will pay in a payment period
				$pdadta .=  '&CURRENCYCODE=USD';    #The currency, e.g. US dollars
				$pdadta .=  '&COUNTRYCODE=US';    #The country code, e.g. US
				$pdadta .=  '&MAXFAILEDPAYMENTS=3';
		...
		function GetTransactionDetails(){
			...
			if("SUCCESS" == strtoupper($httpParsedResponseAr["ACK"]) || "SUCCESSWITHWARNING" == strtoupper($httpParsedResponseAr["ACK"])){
				...
				$this->SaveBuyerInfoIntoDatabase($FIRSTNAME, $LASTNAME, $EMAIL);
